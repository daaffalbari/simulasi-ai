...
def perform_inference(age, business_travel, hourly_rate, overtime):
    ...
    pengurangan_karyawan = ''
    
    def menentukan_pengurangan_karyawan_yes(z):
        if z <= 237:
            return 0
        elif 237 < z <= 1233:
            return (1233 - z) / (1233 - 237)
        else:
            return 0
        
    def menentukan_pengurangan_karyawan_no(z):
        if z <= 237:
            return 1
        elif 237 < z <= 1233:
            return (z - 237) / (1233 - 237)
        else:
            return 1
        
    pengurangan_karyawan_yes_val = menentukan_pengurangan_karyawan_yes(nilai_setara)
    pengurangan_karyawan_no_val = menentukan_pengurangan_karyawan_no(nilai_setara)

    if pengurangan_karyawan_yes_val < pengurangan_karyawan_no_val:
      pengurangan_karyawan = 'Karyawan mengalami pengurangan'
    else:
      pengurangan_karyawan = 'Karyawan tidak mengalami pengurangan'

    return pengurangan_karyawan

# Streamlit app
def main():
    st.title("Employee Reduction Prediction App✨")

    # Input sliders
    age = st.slider("Select Age:", min_value=0, max_value=100, value=37, step=1)
    business_travel = st.slider("Business Travel Frequency:", min_value=0, max_value=10, value=3, step=1)
    hourly_rate = st.slider("Hourly Rate:", min_value=0, max_value=100, value=32, step=1)
    overtime = st.slider("Overtime Hours:", min_value=0, max_value=10, value=6, step=1)

    # Submit button
    if st.button("Submit"):
        # Perform inference when the button is clicked
        result = perform_inference(age, business_travel, hourly_rate, overtime)

        # Display result
        st.subheader("Prediction Result:")
        st.markdown(f"Hasil Prediksinya adalah: **{result}**")
